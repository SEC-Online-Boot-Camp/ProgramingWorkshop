...
def test_04_Update_01(client):
    """GET /tasks/edit/<id> で編集フォームが表示されること"""
    response = client.get("/tasks/edit/1")
...
def test_04_Update_02(client):
    """POST /tasks/update/<id> でタスクの状態を更新できること"""
...
def test_04_Update_03(client):
    """POST /tasks/update/<id> で無効な状態を指定した場合は400エラーになること"""
    response = client.post(
        "/tasks/update/1",
        data={"status": "無効な状態"},
    )
...
def test_04_Update_04(client):
    """GET /tasks/edit/<id> で存在しないIDを指定した場合は404エラーになること"""
    response = client.get("/tasks/edit/9999")
