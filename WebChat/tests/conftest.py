...
try:
    ...
    app_mod = None
webchat_mod = types.ModuleType("WebChat")
webchat_mod.app = app_mod
sys.modules["WebChat"] = webchat_mod
...
def client(tmp_path, monkeypatch):
    ...
    # テスト用DBを新規作成（毎回初期化）
    test_db = tmp_path / "test_webchat.db"
    ...
    conn.execute(
        ...
        INSERT INTO messages (
            created_at, message, status, assigned, priority, due_date, description
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (
            now,
